...
def cstr_ode(x, u):
    X, S, V = x[0], x[1], x[2]
    dV_dt = F_in - F_out
    dP_dt = (-F_out * P - dV_dt * P) / V + (k_fwd * S) / V - (k_bwd * P) / V
    dS_dt = (F_in * S_in - F_out * S - dV_dt * S) / V - (k_fwd * S) / V + (k_bwd * P) / V
    
    return ca.vertcat(dP_dt, dS_dt, dV_dt)
...
casadi_model = CasadiModel(
    ...
    controls=u,
    ode=cstr_ode,
    process_noise=np.diag([0.00, 0.00, 0.00]),
    init_state=X0,
    dt=dt,
    T=T
)
...
initial_state_guess = np.array([1.0, 0.0, 1.0])  # Slightly perturbed initial guess

enkf_casadi = EnKalmanFilter(
    model=casadi_model,
    ensemble_size=50,
    initial_covariance=np.diag([0.2, 0.2, 0.0]),
...
cadet_model_enkf = CadetModel(
    ...
    n_states=2,
    n_controls=0,
    process_noise=np.diag([0.00, 0.00]),
